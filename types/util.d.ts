...
export namespace util {
  ...
  namespace CQCode {
    ...
    class CQCodeString extends String {
      /** 转换为对象形式 */
      parse(this: string): CQCode.CQCodeData;
    }
    type CQCodeData = {
      type: string;
      data: {
     ...
     * @param id 表情编号
     */
    function face(id: number): CQCodeString;
    /**
     * emoji表情
     * @param id unicode编号
     */
    function emoji(id: number): CQCodeString;
    /**
     * 原创表情
     * @param id 该原创表情的ID，存放在酷Q目录的data\bface\下
     */
    function bface(id: number): CQCodeString;
    /**
     * 小表情
     * @param id 该小表情的ID
     */
    function sface(id: number): CQCodeString;
    /**
     * 自定义图片
     * @param file 图片文件名称，图片存放在酷Q目录的data\image\下
     */
    function image(file: string): CQCodeString;
    /**
     * 语音
     * @param file 音频文件名称，音频存放在酷Q目录的data\record\下
     * @param magic 是否为变声，若该参数为true则显示变声标记
     */
    function record(file: string, magic?: boolean): CQCodeString;
    /**
     * at
     * @param qq `被@的群成员帐号。若该参数为all，则@全体成员（次数用尽或权限不足则会转换为文本）`
     */
    function at(qq: number): CQCodeString;
    /**
     * 猜拳魔法表情
     ...
     * - 3 猜拳结果为布
     */
    function rps(type?: 1 | 2 | 3): CQCodeString;
    /**
     * 掷骰子魔法表情
     * @param type 掷出的点数，暂不支持发送时自定义
     */
    function dice(type?: 1 | 2 | 3 | 4 | 5 | 6): CQCodeString;
    /** 戳一戳（原窗口抖动，仅支持好友消息使用） */
    function shake(): CQCodeString;
    /**
     * 匿名发消息（仅支持群消息使用）,本CQ码需加在消息的开头
     * @param ignore 允许忽略匿名，若为`true`，匿名失败将转为普通消息发送，若为`false`或空，匿名失败将取消该消息的发送
     */
    function anonymous(ignore?: boolean): CQCodeString;
    /**
     * 发送音乐
     * @param type 音乐平台类型
     * @param id 对应音乐平台的数字音乐id
     */
    function music(type: 'qq' | '163' | 'xiami', id: number): CQCodeString;
    /**
     * 发送音乐自定义分享
     ...
     */
    function music(type: 'custom', url: string, audio: string, title: string, content?: string, image?: string): CQCodeString;
    /**
     * 发送链接分享
     * @param url 分享链接
     ...
     */
    function share(url: string, title: string, content?: string, image?: string): CQCodeString;
  }
}